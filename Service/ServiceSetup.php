...
class ServiceSetup extends SetupModule
{
    public function setup() {
        ...
        $param['rows'] = 5;
        $param['value'] = '';
        $this->addDefault('TEXTAREA','SRV_EMAIL_FOOTER','Email footer',$param);

        /* These settings were only relevant to old invoice PDF setup
        $param = [];
        $param['info'] = 'Specify invoice footer text / bank account details / any info you require to be added.';
        $param['rows'] = 10;
        $param['value'] = '';
        $this->addDefault('TEXTAREA','SRV_INVOICE_FOOTER','Invoice PDF footer',$param);
        ...
        $this->addDefault('IMAGE','SRV_INVOICE_SIGN','Invoice signature',$param);

        $param = [];
        $param['info'] = 'Specify the name and title you wish to have below signature.';
        $param['value'] = 'Chief Executive Officer';
        $this->addDefault('TEXT','SRV_INVOICE_SIG_TXT','Invoice signature subtext',$param);
        */
    }    
}
